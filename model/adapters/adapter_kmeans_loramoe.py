...
class Adapter(nn.Module):
    def __init__(self, vit_name, num_quires, fusion_map, mlp_dim, mlp_out_dim, head_num, device):
        ...
        cluster_dir = Path("model/clusters/")
        for i, path in enumerate(cluster_dir.glob(f'vit_block{i}_n*.pt')):  # Assuming cluster_dir is a Path object
            centers = torch.load(path, map_location=self.device)  # shape: [n_clusters, dim]
            self.lora_moe_layers[i].set_cluster_centers(centers)
